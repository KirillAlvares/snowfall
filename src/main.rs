...
impl World {
    fn new(width: f64, height: f64) -> World {
        World {
            current_turn: 0,
            particies: Vec::<Box<Particle>>::new(),
            height: height,
            width: width,
            rng: thread_rng(),
        }
    ...
    fn add_shapes (&mut self, n: i32) {
        for _ in 0..n.abs() {
            ...
            self.particies.push(boxed_particle);
        }
    }

    fn remote_shapes (&mut self, n:i32) {
        for _ in 0..n.abs() {
            let mut to_delete =None;

            let particle_iter = self.particies.iter().enumerate();
            ...
            if let Some(i) = to_delete {
                self.particies.remove(i);}
                else{
                    self.particies.remove(0);
                };
            }        
        }
        ...
        fn update(& mut self) {
            ...
            if n > 0 {
                ...
                self.remote_shapes(n);
            }

            self.particies.shrink_to_fit();
            for shape in &mut self.particies {
                shape.update();
            }
            self.current_turn += 1;
        }
